...
namespace Eagle.Repositories.Sql
{
    ...
    public class SqlRepositoryContext : RepositoryContext, ISqlRepositoryContext
    {
        ...
        {
            this.database = new Database(Eagle.Data.DbProviderFactory.CreateDbProvider(connectionStringSectionName)); 
        }

        public SqlRepositoryContext(DatabaseType databaseType, string connectionString) 
        {
            string dbProviderTypeName = typeof(Eagle.Data.SqlServer.SqlServerDbProvider).FullName;

            if (databaseType == DatabaseType.SqlServer)
            {
                dbProviderTypeName = typeof(Eagle.Data.SqlServer.SqlServerDbProvider).FullName;
            }
            else if (databaseType == DatabaseType.Oracle)
            {
                dbProviderTypeName = typeof(Eagle.Data.Oracle.OracleDbProvider).FullName;
            }
            else if (databaseType == DatabaseType.MySql)
            {
                dbProviderTypeName = typeof(Eagle.Data.MySql.MySqlDbProvider).FullName;
            }
            else if (databaseType == DatabaseType.SqlLite)
            {
                dbProviderTypeName = typeof(Eagle.Data.SqlLite.SqlLiteDbProvider).FullName;
            }

            this.database = new Database(Eagle.Data.DbProviderFactory.CreateDbProvider(null, dbProviderTypeName, connectionString));
        }
        ...
        public string GetOrderByClauseSql<TAggregateRoot>(Expression<Func<TAggregateRoot, object>> predicate) where TAggregateRoot : class, new()
        {
        ...
        private string BuildParameterPrefix(string sqlCommandText)
        {
            if (string.IsNullOrEmpty(sqlCommandText) ||
                sqlCommandText.IsNullOrWhiteSpace())
            {
                return null;
            }
