...
namespace Eagle.Core.IoC
{
    public class UnityObjectContainer : IObjectContainer
    {
        ...
        {
            UnityConfigurationSection unityConfigSection = null;

            if (configFilePath.HasValue())
            {
                unityConfigSection = ConfigurationUtils.GetConfigurationSection<UnityConfigurationSection>(configFilePath, configSectionName);
            }
            else
            {
                unityConfigSection = (UnityConfigurationSection)ConfigurationManager.GetSection(configSectionName);
            }

            unityConfigSection.Configure(this.unityContainer);
        }
        ...
        {
            ParameterOverride[] parameterOverrides = this.ConvertToParameterOverrides(parameters);

            return this.unityContainer.Resolve<T>(parameterOverrides);
        }
        ...
        public T Resolve<T>(string name, params ResolverParameter[] parameters)
        {
            ...
            return this.unityContainer.Resolve<T>(name, parameterOverrides);
        }
        ...
        private ParameterOverride[] ConvertToParameterOverrides(ResolverParameter[] parameters)
        {
            if (parameters != null)
            {
                return parameters.Select(p => new ParameterOverride(p.ParameterName, p.ParameterValue)).ToArray();
            }
